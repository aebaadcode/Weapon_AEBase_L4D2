datablock AudioProfile(L4D2_Mac10FireSound)
{
   filename    = "./Sounds/Fire/Mac10_fire.wav";
...
// item //
//////////
datablock ItemData(L4D2_Mac10Item)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Mac10/Mac10PU.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
	...
	//gui stuff
	uiName = "L4D2: Mac10";
	iconName = "./Icons/UI_Mac10";
	doColorShift = true;
	colorShiftColor = "0.4 0.4 0.41 1.000";

	 // Dynamic properties defined by the scripts
	image = L4D2_Mac10Image;
	canDrop = true;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(L4D2_Mac10Image)
{
   // Basic Item properties
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = true;
	safetyImage = L4D2_Mac10SafetyImage;
//    scopingImage = L4D2_Mac10IronsightImage;
	doColorShift = true;
	colorShiftColor = L4D2_Mac10Item.colorShiftColor;//"0.400 0.196 0 1.000";
...
function L4D2_Mac10Image::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(3); 
    %obj.playAudio(3, L4D2_Mac10FireSound);
...
function L4D2_Mac10Image::AEOnLowClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(0, plant);
}

function L4D2_Mac10Image::AEOnMedClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(3, jump);
   %obj.aeplayThread(0, plant);
}

function L4D2_Mac10Image::AEOnHighClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(3, jump);
}

function L4D2_Mac10Image::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function L4D2_Mac10Image::onReady(%this,%obj,%slot)
{
...
function L4D2_Mac10Image::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function L4D2_Mac10Image::onUnMount(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(L4D2_Mac10SafetyImage)
{
   shapeFile = "./Mac10/Mac10.dts";
   emap = true;
   mountPoint = 0;
   ...
   armReady = false;
   hideHands = true;
   scopingImage = L4D2_Mac10Image;
   safetyImage = L4D2_Mac10Image;
   doColorShift = true;
   colorShiftColor = L4D2_Mac10Item.colorShiftColor;
...
function L4D2_Mac10SafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function L4D2_Mac10SafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
	cancel(%obj.reload3Schedule);
	cancel(%obj.reload4Schedule);
...
function L4D2_Mac10SafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
	parent::onUnMount(%this,%obj,%slot);
}