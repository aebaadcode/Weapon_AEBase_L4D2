datablock AudioProfile(L4D2_UziFireSound)
{
   filename    = "./Sounds/Fire/Uzi_fire.wav";
...
// item //
//////////
datablock ItemData(L4D2_UziItem)
{
	category = "Weapon";  // Mission editor category
	className = "Weapon"; // For inventory system

	 // Basic Item Properties
	shapeFile = "./Uzi/UziPU.dts";
	rotate = false;
	mass = 1;
	density = 0.2;
	elasticity = 0.2;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(L4D2_UziImage)
{
   // Basic Item properties
   shapeFile = "./Uzi/Uzi.dts";
   emap = true;
   ...
   //raise your arm up or not
	armReady = true;
	hideHands = true;
	safetyImage = L4D2_UziSafetyImage;
 //   scopingImage = L4D2_UziIronsightImage;
	doColorShift = true;
	colorShiftColor = L4D2_UziItem.colorShiftColor;//"0.400 0.196 0 1.000";
   ...
   // Initial start up state
	...
	stateSound[0]				= L4D2_UziEquipSound;
	stateSequence[0]			= "use";
...
function L4D2_UziImage::AEOnFire(%this,%obj,%slot)
{	
	%obj.stopAudio(3); 
    %obj.playAudio(3, L4D2_UziFireSound);
...
function L4D2_UziImage::AEOnLowClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(0, plant);
}

function L4D2_UziImage::AEOnMedClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(3, jump);
   %obj.aeplayThread(0, plant);
}

function L4D2_UziImage::AEOnHighClimb(%this, %obj, %slot) 
{
   %obj.aeplayThread(3, jump);
}

function L4D2_UziImage::onDryFire(%this, %obj, %slot)
{
	%obj.aeplayThread(2, plant);
	serverPlay3D(AEDryFireSound, %obj.getHackPosition());
}

function L4D2_UziImage::onReady(%this,%obj,%slot)
{
...
function L4D2_UziImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function L4D2_UziImage::onUnMount(%this,%obj,%slot)
{
...
datablock ShapeBaseImageData(L4D2_UziSafetyImage)
{
   shapeFile = "./Uzi/Uzi.dts";
   emap = true;
   mountPoint = 0;
   ...
   armReady = false;
   hideHands = true;
   scopingImage = L4D2_UziImage;
   safetyImage = L4D2_UziImage;
   doColorShift = true;
   colorShiftColor = L4D2_UziItem.colorShiftColor;
...
function L4D2_UziSafetyImage::onDone(%this,%obj,%slot)
{
	%obj.mountImage(%this.safetyImage, 0);
}

function L4D2_UziSafetyImage::onMount(%this,%obj,%slot)
{
	%this.AEMountSetup(%obj, %slot);
	cancel(%obj.reload3Schedule);
	cancel(%obj.reload4Schedule);
...
function L4D2_UziSafetyImage::onUnMount(%this, %obj, %slot)
{
	%this.AEUnmountCleanup(%obj, %slot);
	parent::onUnMount(%this,%obj,%slot);
}
...
function L4D2_UziIronsightImage::onMount(%this,%obj,%slot)
{
	%obj.aeplayThread(2, plant);
...
function L4D2_UziIronsightImage::onUnMount(%this,%obj,%slot)
{
	if(isObject(%obj.client) && %obj.client.IsA("GameConnection"))
